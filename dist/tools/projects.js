...
function formatProject(project) {
    ...
    if (project.parent_id)
        lines.push(`- **Parent ID**: ${project.parent_id}`);
...
export function registerProjectTools(server) {
    ...
    server.registerTool("todoist_get_projects", {
...
Use project IDs with todoist_get_tasks to fetch tasks for a specific project.`,
    ...
    // ── todoist_get_project ─────────────────────────────────────────────────────
    server.registerTool("todoist_get_project", {
...
comment count, sharing status, and view style.`,
        ...
        try {
            const project = await apiGet(`/projects/${params.project_id}`);
            let text;
            if (params.response_format === ResponseFormat.JSON) {
    ...
    server.registerTool("todoist_create_project", {
...
Returns the full created project object including its assigned ID.`,
        inputSchema: z.object({
            ...
            color: z
                .string()
                .optional()
                .describe("Project color name (e.g. 'blue', 'red', 'green'). See description for full list."),
            is_favorite: z
                .boolean()
                .optional()
        ...
        try {
            ...
            if (params.view_style)
                body.view_style = params.view_style;
            const project = await apiPost("/projects", body);
    ...
    server.registerTool("todoist_update_project", {
        ...
        try {
            ...
            if (params.is_favorite !== undefined)
                body.is_favorite = params.is_favorite;
            if (params.view_style !== undefined)
                body.view_style = params.view_style;
            ...
            const project = await apiPost(`/projects/${params.project_id}`, body);
    ...
    server.registerTool("todoist_delete_project", {
