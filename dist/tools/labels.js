...
export function registerLabelTools(server) {
    // ── todoist_get_labels ──────────────────────────────────────────────────────
    server.registerTool("todoist_get_labels", {
...
Use label names (not IDs) when creating or updating tasks.`,
    ...
    server.registerTool("todoist_create_label", {
...
when assigning labels to tasks with todoist_create_task or todoist_update_task.`,
        inputSchema: z.object({
            ...
            color: z
                .string()
                .optional()
                .describe("Label color name (e.g. 'blue', 'red', 'green', 'orange', 'grape')"),
            order: z
                .number()
                .int()
                .optional()
        ...
        try {
            ...
            const label = await apiPost("/labels", body);
            return {
                content: [
                    {
                        type: "text",
    ...
    server.registerTool("todoist_update_label", {
        ...
        try {
            ...
            const label = await apiPost(`/labels/${params.label_id}`, body);
            return {
                content: [
                    {
                        type: "text",
    ...
    server.registerTool("todoist_delete_label", {
