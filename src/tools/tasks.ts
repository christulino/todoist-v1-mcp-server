...
interface Task {
  ...
  [key: string]: unknown; // allow additional API fields gracefully
}
...
function formatTask(task: Task): string {
  ...
  if (task.due) lines.push(`- **Due**: ${task.due.string ?? task.due.date}`);
  if (task.priority > 1)
  ...
  if (task.section_id) lines.push(`- **Section ID**: ${task.section_id}`);
  if (task.url) lines.push(`- **URL**: ${task.url}`);
  return lines.join("\n");
}

function formatTaskList(tasks: Task[], title: string): string {
...
Returns task IDs, content, due dates, priorities, labels, and project/section IDs.`,
      inputSchema: z.object({
        filter: z
          ...
          .optional()
          .describe(
            "Todoist filter string, e.g. 'today', 'overdue', 'p1', '#ProjectName', '@label', '7 days'"
          ),
        project_id: z
          .string()
          .optional()
    ...
    async (params) => {
      try {
        ...
        const data = await apiGet<TaskListResponse>("/tasks", query);
...
project/section IDs, comment count, and creation timestamp.`,
    ...
    async (params) => {
      try {
        const task = await apiGet<Task>(`/tasks/${params.task_id}`);
...
Returns the full created task object including its assigned ID.`,
      inputSchema: z.object({
        ...
        project_id: z
          ...
          .optional()
          .describe(
            "Project ID to add the task to (defaults to Inbox if omitted)"
          ),
        section_id: z
          .string()
          .optional()
        ...
        due_string: z
          ...
          .optional()
          .describe(
            "Natural language due date: 'tomorrow', 'every monday', 'Feb 20', 'next week'"
          ),
        due_date: z
          .string()
          .optional()
    ...
    async (params) => {
      try {
        ...
        const task = await apiPost<Task>("/tasks", body);
...
Priority: 1=normal, 2=medium, 3=high, 4=urgent.`,
    ...
    async (params) => {
      try {
        ...
        const task = await apiPost<Task>(`/tasks/${params.task_id}`, body);
...
Use todoist_delete_task to permanently remove a task instead.`,
    ...
    async (params) => {
      try {
        await apiPost<void>(`/tasks/${params.task_id}/close`);
        return {
          content: [
            {
              type: "text",
...
Useful for tasks that were marked done by mistake or need to be revisited.`,
    ...
    async (params) => {
      try {
        await apiPost<void>(`/tasks/${params.task_id}/reopen`);
        return {
          content: [
            {
              type: "text",
