...
todoist_create_task (section_id param) to work with tasks in specific sections.`,
    ...
    async (params) => {
      try {
        const data = await apiGet<Section[] | { items?: Section[]; results?: Section[] }>(
          "/sections",
          { project_id: params.project_id }
        );

        const sections: Section[] = Array.isArray(data)
          ? data
...
creating tasks (todoist_create_task) to add them directly to this section.`,
    ...
    async (params) => {
      try {
        ...
        const section = await apiPost<Section>("/sections", body);
  ...
  server.registerTool(
    ...
    async (params) => {
      try {
        const section = await apiPost<Section>(`/sections/${params.section_id}`, {
          name: params.name,
        });
